...
export function SignalListPanel() {
  ...
  const typeColor: Record<string, string> = {
    ...
    WATCH: 'border-l-slate-500 bg-secondary/30',
  };

  return (
    <div className="space-y-1 p-1">
      <div className="flex items-center gap-2 px-1 mb-1">
        <span className="text-[10px] text-muted-foreground">共 {currentSignals.length} 個</span>
        <span className="text-[10px] text-muted-foreground/60">門檻:</span>
        {[0, 55, 65, 75].map(v => (
          <button key={v} onClick={() => setSignalThreshold(v)}
            className={`text-[10px] px-1.5 py-0.5 rounded ${signalThreshold === v ? 'bg-sky-600 text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
            {v === 0 ? '全部' : `≥${v}`}
          </button>
        ))}
        <span className="text-[10px] text-sky-400 ml-auto">{filtered.length} 筆</span>
      </div>
      {recent.length === 0 && <div className="text-xs text-muted-foreground/60 text-center py-8">無符合條件的訊號</div>}
      {recent.map(sig => (
        <div key={sig.id} className={`border-l-2 rounded-r-lg p-2 text-xs ${typeColor[sig.type] ?? typeColor.WATCH}`}>
          <div className="flex items-center gap-1.5">
            <span className={`font-black text-[10px] px-1 rounded ${
              sig.type === 'BUY' ? 'bg-red-700 text-foreground' :
              sig.type === 'SELL' ? 'bg-green-700 text-foreground' :
              'bg-muted text-foreground/80'
            }`}>{sig.type}</span>
            <span className="font-bold text-foreground">{sig.label}</span>
            <span className="text-muted-foreground/60 text-[10px]">{sig.timeframe}</span>
            <span className="ml-auto text-[10px] bg-secondary px-1 rounded">{sig.score}</span>
          </div>
          <div className="mt-1 text-muted-foreground text-[11px]">{sig.reason}</div>
          <div className="mt-0.5 flex items-center gap-2 text-[10px] text-muted-foreground/60 flex-wrap">
            <span>{sig.triggeredAt.split('T')[1]?.slice(0,5)} @ {sig.price.toFixed(2)}</span>
            {sig.metadata.stopLossPrice && (
              <span className="text-bear">止損 {sig.metadata.stopLossPrice.toFixed(1)}</span>
            )}
            {sig.metadata.targetPrice && (
              <span className="text-bull">目標 {sig.metadata.targetPrice.toFixed(1)}</span>
            )}
