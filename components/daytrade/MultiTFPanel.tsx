...
export function MultiTFPanel() {
  ...
  if (!mtfState) return <div className="text-xs text-muted-foreground/60 p-4 text-center">載入數據後顯示</div>;

  const tfs: IntradayTimeframe[] = ['60m', '15m', '5m', '1m'];
  const labels: Record<string, string> = { '60m': '大方向', '15m': '結構強弱', '5m': '進出節奏', '1m': '確認時機' };
  const biasColor = mtfState.overallBias === 'bullish' ? 'text-bull bg-red-900/40' :
                    mtfState.overallBias === 'bearish' ? 'text-bear bg-green-900/40' :
  ...
  return (
    <div className="space-y-3 p-1">
      ...
      {tfs.map(tf => {
        ...
        return (
          <div key={tf} className="bg-secondary/50 rounded-lg p-2.5 space-y-1">
            <div className="flex items-center gap-2 text-xs">
              <span className="font-bold text-foreground">{tf}</span>
              <span>{icon}</span>
              <span className={s.trend === 'bullish' ? 'text-bull' : s.trend === 'bearish' ? 'text-bear' : 'text-yellow-400'}>
                {trendLabel}
              </span>
              <span className="ml-auto text-muted-foreground text-[10px]">{labels[tf]}</span>
            </div>
            <div className="flex items-center gap-1">
              ...
              <span className="text-[10px] text-muted-foreground w-6 text-right">{s.trendStrength}</span>
            </div>
